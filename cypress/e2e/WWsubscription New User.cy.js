// New user Register in WW New subscription plan.

// ExiSting user Available in WW
describe('WW Subscriptions Plan', () => {
    ...
    it('Completes full subscription flow using 2nd radio options', () => {
      ...
      //cy.wait(2000);
      //cy.get('input[id="enter_email"]').type('man2420@test.io');
...
// wait for either login_password OR create_password field
cy.wait(2000);

// Check if "Create Password" field is visible
cy.get('body').then(($body) => {
  if ($body.find('input[id="create_password"]').length) {
    // New user flow: create password
    cy.get('input[id="create_password"]').type('dual1983');
    cy.get('input[id="confirm_password"]').type('dual1983');
    cy.get('button').contains('Continue').click();
  } else {
...
// Optional: Verify final redirect
// cy.url().should('include', '/dashboard/weightloss');
